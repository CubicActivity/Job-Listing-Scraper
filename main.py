...
if __name__ == "__main__":
    ...
    print(f"Single process collected {len(serial_jobs)} jobs in {serial_time:.2f}s")

    # 4 processes benchmark
    t1 = time.time()
    parallel_jobs = fetch_jobs(fetchers, processes=4)
    parallel_time = time.time() - t1
    print(f"Parallel (4 process) collected {len(parallel_jobs)} jobs in {parallel_time:.2f}s")
