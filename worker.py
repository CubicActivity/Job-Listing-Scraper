...
async def process_task(session, task):
        ...
        try:
            ...
            if not jobs:
                ...
                for row in job_rows:
                    ...
                    link_el = (
                        row.select_one("a[rel='cannonical']")
                        or row.select_one("a[href*='/jobs/']")
                    )
    ...
    # ARBEITNOW
        ...
        url = (
            ...
            f"?search=&tags=&sort_by=null&page={page}"
        )
...
async def process_and_store(session, task):
